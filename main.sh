...
def show_device_info():
    ...
    cpu = get_processor()
    print(color("  ▪︎ Processor", 'white') + color(f"  {cpu}", 'dim'))
    
    print(color("  ▪︎ Memory", 'white') + color(f"     {storage}", 'dim'))
    
    screen = get_screen_info()
    print(color("  ▪︎ Screen", 'white') + color(f"      {screen}", 'dim'))
    ...
    print(color("  ▪︎ Serial", 'white') + color(f"       {serial}", 'dim'))
    
    imei = get_device_info("InternationalMobileEquipmentIdentity")
    if not show_sensitive:
        imei = mask_string(imei)
    print(color("  ▪︎ IMEI", 'white') + color(f"         {imei}", 'dim'))
    
    print()
    print(color("  Software", 'gray'))
    print()
    
    print(color("  ▪︎ Jailbreak", 'white') + color("    N/A, Un-support", 'dim'))
...
def main():
    ...
    try:
        ...
        if not result.stdout.strip():
            print(color("\n❌ Error: libimobiledevice not installed!", 'red'))
            print(color("   Install it with: brew install libimobiledevice\n", 'yellow'))
            sys.exit(1)
    except:
        print(color("\n❌ Error: Cannot check for libimobiledevice\n", 'red'))
        sys.exit(1)
    ...
    print(color("\n🔧 fourtools v1.0", 'cyan') + color(" - made with love <3\n", 'dim'))
    time.sleep(1)
